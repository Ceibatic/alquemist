...
## [11:22] inventory — Lot status filter dropdown
- **Files:** `components/inventory/inventory-list.tsx`
- **Why:** Add lot status filtering to improve inventory search and filtering capabilities.
- **Commit:** `abf6260`

**Cambios implementados:**
1. New lotStatusFilter state: useState<string[]>([])
2. lotStatusOptions array with 5 status types:
   - available (Disponible) - CheckCircle2 icon
   - reserved (Reservado) - Lock icon
   - expired (Expirado) - AlertCircle icon
   - quarantine (Cuarentena) - AlertTriangle icon
   - discontinued (Descontinuado) - XCircle icon
3. Filter UI in existing Popover:
   - New "Estado de Lote" section with checkboxes
   - Dynamic rendering using .map() over lotStatusOptions
   - Consistent styling with existing stock status filter
4. Filter logic in filteredItems useMemo:
   - Applied after stock status filter
   - Filters items by lot_status field
   - Only active when lotStatusFilter.length > 0
5. Badge counter integration:
   - Added lotStatusFilter.length to activeFiltersCount
   - Badge shows total active filters (stock + lot status)
6. Clear filters:
   - Updated clearAllFilters() to reset lotStatusFilter to []
   - Clears lot status when "Limpiar" button clicked
...
- **Commits:** `eab4d5a`, `65ac773`, `fa0cd59`, `22d24b9`, `4d6266a`, `9c5428d`, `998eda2`, `0babd13`, `77faef0`, `abf6260`, `2ecf39d`
