...
- [x] Se eliminan 5 mutations de `convex/activityTemplates.ts`: `getChecklist`, `addChecklistItem`, `updateChecklistItem`, `removeChecklistItem`, `reorderChecklist`
- [x] Se elimina el archivo `components/activity-templates/checklist-editor.tsx`
- [x] Se elimina la seccion de checklist en `components/activity-templates/wizard-step-fields.tsx` (la referencia al toggle "checklist" en form_fields)
- [x] Se elimina el conteo de checklist en `components/activity-templates/activity-template-card.tsx`
- [x] La tabla `activity_template_checklist` en schema.ts se marca con comentario `// DEPRECATED — dead code, no se renderiza en ejecucion` pero NO se elimina (requiere migracion de datos)
- [x] El wizard de templates funciona correctamente sin la seccion de checklist
...
#### Criterios de Aceptacion
- [x] Se elimina `completeScheduledWithTemplate` de `convex/activities.ts` (~242 lineas, confirmado sin importaciones frontend)
- [x] Se elimina `executeScheduledAsNew` de `convex/activities.ts` (~133 lineas, confirmado sin importaciones frontend)
- [x] Ningun componente frontend importa estas mutations (verificado con grep)
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Campo `group_id: v.optional(v.string())` agregado a `scheduled_activities` — UUID compartido para agrupar actividades multi-batch
- [x] Campo `source: v.optional(v.string())` agregado a `scheduled_activities` — valores: `"template"` | `"manual"` | `"adhoc"`
- [x] Index `.index("by_group", ["group_id"])` agregado a `scheduled_activities`
- [x] Index `.index("by_parent_activity", ["parent_activity_id"])` agregado a `activities`
- [x] Todos los campos son `v.optional()` — backward compatible, no afecta datos existentes
- [x] `npx convex codegen` genera tipos correctamente
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Nuevo archivo `convex/scheduledActivities.ts` con mutation `createManual`:
...
- [x] Query `listByEntity(entityType, entityId, status?)` para buscar actividades programadas por entidad
- [x] Query `getGroup(groupId)` para cargar todas las actividades de un grupo multi-batch
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Nueva mutation `activities.executeActivity` en `convex/activities.ts` con args:
...
- [x] **Single batch**: crea 1 activity via logica existente de `logV2` (reutilizar, no duplicar)
- [x] **Multi-batch**: crea 1 parent activity (`entity_type: "multi_batch"`) + N child activities (1 por batch con `parent_activity_id` apuntando al parent)
- [x] Si `scheduledActivityId` proporcionado: marca esa scheduled_activity como `completed` con `completed_by`, `actual_end_time`
- [x] Si `groupId` proporcionado: marca TODAS las scheduled_activities del grupo como `completed`
- [x] Consumo de inventario delega a logica FIFO existente
- [x] Distribucion `identical`: mismos recursos por cada child activity
- [x] Distribucion `split_proportional`: cantidad total dividida entre N batches
- [x] Retorna `{ activityId, childActivityIds? }` (activityId = parent si multi-batch, unico si single)
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Archivo `lib/schemas/activity-execution.ts` con Zod schema que valida:
...
- [x] Hook `hooks/use-activity-execution.ts` que:
...
- [x] Pre-fill de `responsibleId` con usuario actual por defecto
- [x] Pre-fill de `activityDate` con fecha de hoy por defecto
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Componente principal `components/activities/activity-execution-sheet.tsx` como Sheet lateral:
...
- [x] Sub-componente `execution-step-activity.tsx` — Paso 1: datos de actividad
...
- [x] Sub-componente `execution-step-quality.tsx` — Paso 2: formulario QC (extraido del report sheet actual)
...
- [x] Sub-componente `resource-editor-inline.tsx` — editor de cantidades de recursos inline
...
- [x] Sub-componente `activity-type-picker.tsx` — selector de tipo para ad-hoc
  - Dropdown con activity_types de la company
  - Al seleccionar tipo, carga templates disponibles para sugerir (opcional)
- [x] Sheet tiene header fijo (titulo + badge de modo), footer fijo (Cancelar + Completar Actividad amber-500)
- [x] Indicador de pasos si hay QC (1. Actividad → 2. Calidad)
- [x] Usa el hook `use-activity-execution` para form state, pre-fill y submit
- [x] Toast de confirmacion al completar, cierre automatico
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Componente `components/activities/schedule-activity-dialog.tsx` como Dialog:
  - Props: `open`, `onOpenChange`, `fromOrderId?`, `fromAreaId?`, `fromPhase?`, `fromBatchId?`, `onScheduled?`
- [x] Zod schema en `lib/schemas/schedule-activity.ts`:
...
- [x] Form con React Hook Form + zodResolver:
...
- [x] Al submit llama `api.scheduledActivities.createManual`
- [x] Toast de confirmacion mostrando cuantas actividades se programaron
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] Componente `components/activities/batch-multi-select.tsx`:
  - Props: `companyId`, `value: string[]`, `onChange: (batchIds: string[]) => void`, `orderId?`, `areaId?`, `phase?`, `facilityId?`, `maxSelection?`, `singleMode?`
- [x] Lista de lotes como checkboxes mostrando: `batch_code`, nombre del cultivar, area/zona, fase actual, cantidad actual
- [x] Filtrado automatico segun props:
...
- [x] Solo muestra lotes con `status: "active"` (excluye archived, harvested, lost)
- [x] Boton "Seleccionar todos" / "Deseleccionar todos" en la lista filtrada
- [x] Modo single (`singleMode=true`): renderiza radio buttons en lugar de checkboxes
- [x] Muestra contador: "N lotes seleccionados de M disponibles"
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] **Orden de produccion** (`app/(dashboard)/production/orders/[id]/page.tsx`):
...
- [x] **Area — History tab** (`components/areas/area-history-tab.tsx`):
...
- [x] **Scheduled Activities** (`app/(dashboard)/scheduled-activities/page.tsx`):
...
- [x] **Today Activities Widget** (`components/dashboard/today-activities-widget.tsx`):
...
#### Criterios de Aceptacion
- [x] Los 3 consumidores del viejo `ActivityReportSheet` usan el nuevo `ActivityExecutionSheet`:
...
- [x] Se elimina `components/activities/activity-report-sheet.tsx`
- [x] Grep confirma cero importaciones de `ActivityReportSheet` o `activity-report-sheet`
- [x] Todos los flujos existentes funcionan igual que antes (regresion):
...
#### Criterios de Aceptacion
- [x] Se agrega comentario `/** @deprecated Use executeActivity instead */` a `completeScheduledActivity` en `convex/activities.ts`
- [x] Ningun componente frontend activo llama directamente a `completeScheduledActivity` (toda ejecucion pasa por `executeActivity`)
- [x] Se identifican y documentan queries en `convex/activities.ts` que ya no tienen consumidores frontend (candidatas para eliminacion futura)
- [x] `npx next build` pasa sin errores
...
#### Criterios de Aceptacion
- [x] **Flujo 1 — Template-driven**: Crear template → generar schedule → ejecutar scheduled activity → actividad creada con recursos, scheduled marcada como completed
- [x] **Flujo 2 — Manual schedule**: Abrir dialog desde orden de produccion → seleccionar 1 batch → programar → ejecutar → verificar actividad creada
- [x] **Flujo 3 — Ad-hoc**: Abrir execution sheet sin contexto → seleccionar tipo → llenar campos → submit → actividad creada sin scheduled_activity asociada
- [x] **Flujo 4 — Multi-batch**: Programar desde orden con 3 batches → ejecutar grupo → verificar 1 parent activity + 3 child activities en DB con `parent_activity_id`
- [x] **Flujo 5 — QC**: Ejecutar actividad con template que tiene `quality_check_template_id` → completar paso 1 → completar paso 2 QC → verificar `quality_checks` record creado
- [x] **Flujo 6 — Skip**: Saltar scheduled activity con razon → verificar status "skipped" y `skipped_reason`
- [x] Consumo de inventario FIFO funciona correctamente en single-batch y multi-batch
- [x] `npx next build` pasa sin errores
...
### Commits
- `68ed0ac` — refactor(activity-templates): US-ACT3.1 remove dead checklist code
- `8167070` — refactor(activities): US-ACT3.2 remove unused backend mutations
- `0c9d3ba` — feat(schema): US-ACT3.3 add multi-batch fields and indexes
- `b27ef03` — feat(scheduled-activities): US-ACT3.4 manual scheduling mutation
- `f1e3352` — feat(activities): US-ACT3.5 unified executeActivity mutation
- `d01c34b` — feat(activities): US-ACT3.6 Zod schema and execution hook
- `e708d0e` — feat(activities): US-ACT3.9 BatchMultiSelect component
- `1db9c19` — feat(activities): US-ACT3.7 modular ActivityExecutionSheet
- `f2bc95d` — feat(activities): US-ACT3.8 schedule activity dialog
- `507ef30` — feat(activities): US-ACT3.10 integrate new execution sheet in all entry points
- `8a35975` — refactor(activities): US-ACT3.11 delete old ActivityReportSheet
- `6dc0d60` — docs(activities): US-ACT3.12 deprecate completeScheduledActivity mutation
- `cd5d13b` — fix(activities): US-ACT3.13 e2e validation fixes

### Archivos Modificados
- `convex/schema.ts` — group_id, source fields + by_group, by_parent_activity indexes
- `convex/activities.ts` — executeActivity mutation, removed dead mutations
- `convex/activityTemplates.ts` — removed checklist mutations
- `convex/scheduledActivities.ts` — NEW: createManual, listByEntity, getGroup
- `hooks/use-activity-execution.ts` — NEW: React Hook Form + Zod hook
- `lib/validations/activity-execution.ts` — NEW: Zod schema
- `lib/validations/schedule-activity.ts` — NEW: Zod schema
- `components/activities/activity-execution-sheet.tsx` — NEW: modular Sheet
- `components/activities/execution-step-activity.tsx` — NEW: activity form
- `components/activities/execution-step-quality.tsx` — NEW: QC form
- `components/activities/resource-editor-inline.tsx` — NEW: resource editor
- `components/activities/activity-type-picker.tsx` — NEW: type dropdown
- `components/activities/batch-multi-select.tsx` — NEW: batch selector
- `components/activities/schedule-activity-dialog.tsx` — NEW: scheduling dialog
- `components/activities/activity-report-sheet.tsx` — DELETED (881 lines)
- `components/activity-templates/checklist-editor.tsx` — DELETED
- `components/areas/area-history-tab.tsx` — migrated to new components
- `components/dashboard/today-activities-widget.tsx` — migrated to new components
- `app/(dashboard)/scheduled-activities/page.tsx` — migrated to new components
- `app/(dashboard)/production/orders/[id]/page.tsx` — migrated to new components
