...
- [x] Las rutas `/activity-templates` y `/quality-checks` redirigen a `/templates?tab=activities` y `/templates?tab=quality` respectivamente (backward compatibility)
...
- [x] En la lista de recursos seleccionados, cada item muestra inline: nombre del producto, input de cantidad (number), selector de base (fijo/por planta/por m2/por zona/por L solucion), selector de direccion (consumido/aplicado/producido), boton de eliminar
...
- [x] Los recursos se muestran con subtotales visuales (cantidad x base)
...
- [x] Validacion: campos esenciales requeridos, recursos con cantidad > 0 si estan marcados como requeridos
- [x] Al completar, llama a `api.activities.logV2` mutation con todos los datos
...
- [x] Se puede omitir el paso de calidad con boton "Omitir calidad" si el usuario decide no completar el QC en ese momento
...
- [ ] Al completar el reporte, la scheduled_activity se marca como `completed` automaticamente con `actual_start_time` y `actual_end_time` (nota: requiere hook en logV2 o client-side call)
...
- [ ] En el tab de Produccion del area, cada batch card muestra opcion "Registrar actividad" en su menu contextual (deferred: PhaseCards navigate to phase detail)
...
- [x] La ruta `/activity-templates/[id]` se mantiene funcional (wizard con breadcrumbs actualizados a /templates)
- [x] La ruta `/quality-checks` redirige a `/templates?tab=quality` via Next.js redirect
- [x] La ruta `/quality-checks/templates/[id]` se mantiene funcional con breadcrumbs actualizados a /templates?tab=quality
...
- [x] No quedan links rotos en toda la aplicacion (verificado con build)
...
- `ee51721` — feat(templates): US-TPL.3 wizard resource cart step
- `d5bc31e` — feat(templates): US-TPL.4 wizard final config step
- `bed89b9` — feat(activities): US-TPL.5 activity report sheet base component
- `7ab9a28` — feat(activities): US-TPL.6 quality check step in activity report sheet
- `6411864` — feat(activities): US-TPL.7 integrate report sheet in scheduled activities and areas
- `db06694` — feat(templates): US-TPL.8 navigation cleanup and deprecated routes

### Archivos Modificados
- `app/(dashboard)/templates/page.tsx` — unified 3-tab templates page
- `app/(dashboard)/activity-templates/page.tsx` — redirect to /templates?tab=activities
- `app/(dashboard)/activity-templates/[id]/page.tsx` — wizard-based detail/edit page
- `app/(dashboard)/quality-checks/page.tsx` — redirect to /templates?tab=quality
- `app/(dashboard)/quality-checks/templates/[id]/page.tsx` — updated breadcrumbs
- `app/(dashboard)/quality-checks/inspections/[id]/page.tsx` — updated breadcrumbs
- `app/(dashboard)/scheduled-activities/page.tsx` — added "Reportar" button + template picker
- `components/activity-templates/activity-template-wizard.tsx` — 4-step wizard orchestrator
- `components/activity-templates/wizard-step-basic.tsx` — step 1: type and basic info
- `components/activity-templates/wizard-step-fields.tsx` — step 2: optional form fields
- `components/activity-templates/wizard-step-resources.tsx` — step 3: resource cart
- `components/activity-templates/wizard-step-config.tsx` — step 4: quality, docs, recurrence
- `components/activities/activity-report-sheet.tsx` — lateral sheet with 2-step QC flow
- `components/areas/area-history-tab.tsx` — added "Registrar actividad" button
- `components/layout/sidebar.tsx` — removed deprecated nav items
- `components/layout/sidebar-mobile.tsx` — removed deprecated nav items
- `components/home/operative-dashboard.tsx` — updated QC quick action link
- `convex/schema.ts` — added form_fields, quality_check_template_id, requires_photos, requires_attachments
- `convex/activityTemplates.ts` — updated create/update mutations + getById with resource enrichment
