# Product PRD - Alquemist v4.1
**Streamlined Feature Specifications**

**Version**: 4.1  
**Date**: January 2025  
**Purpose**: Simplified feature specifications optimized for development workflow

*📐 For technical implementation, see [Engineering PRD]*  
...
This PRD focuses on **what** we're building and **why** it matters to users. Each module provides:
- Clear feature descriptions
- User stories from Colombian market perspective
- Success metrics
- Dependencies

Technical implementation details are in the Engineering PRD to avoid redundancy.
...
```
1. Sign Up (basic info) → 2. Email Verification → 
3. Plan Selection & Payment → 4. First Facility Setup → 
5. Crop Type Selection → 6. Sample Data Generation → 
7. Dashboard Ready
```
...
# FASE 1: ONBOARDING MODULES

## MODULE 1: AUTHENTICATION & ACCOUNT CREATION

### Overview
User registration with Colombian business entity support. Creates company account with owner user in single flow. Additional profile details configured post-onboarding.
...
### Dependencies
- Requires: PostgreSQL, Colombian geographic data
...
### Overview
Email confirmation system to verify user accounts before platform access. Prevents fake accounts and ensures reliable communication channel.
...
### Overview
Wompi-based prepaid subscription system for Colombian market. Users select plan, pay monthly in advance via Colombian payment methods. Manual renewal with automated reminders (Wompi doesn't support auto-recurring yet).

### Key Features
- 4 plans: Trial (free 30 days), Starter, Professional, Enterprise
- Wompi payment integration (PSE, cards, Nequi, Bancolombia)
- Manual monthly renewal with 7-day and 1-day reminders
...
**As a System:**
- I want to track usage and enforce plan limits
- I want to handle payment failures gracefully
- I want to prevent service interruption during grace period

### Colombian Market Requirements
- Payment gateway: Wompi (Colombian market leader)
...
- Integrates with: Wompi API, Email service
- Future integration: Colombian electronic invoicing (DIAN)
...
### Overview
Post-onboarding company profile configuration. Users complete legal, contact, and compliance information from settings page. Not required to use platform.
...
- Integrates with: File storage (MinIO/S3)
...
### Overview
Sample cultivars and suppliers pre-loaded for selected crop types. Users can immediately create production orders with realistic data or customize later.
...
# FASE 2: CORE OPERATIONS MODULES

## MODULE 9: INVENTORY MANAGEMENT

### Overview
Unified inventory system across multiple crop types with automated consumption tracking. Tracks products, materials, and equipment with Colombian supplier integration.
...
### Overview
Template-based production workflow system. Pre-configured templates for Colombian crops with automatic activity scheduling and resource allocation.
...
### Overview
AI-powered quality control system with template generation from Spanish documents. Colombian pest/disease detection and batch-level quality assessment.
...
- Compliance documentation automation: 100%

### Dependencies
- Requires: Module 10 (Production Templates), Module 12 (Operations)
- Integrates with: Module 13 (AI Engine), File storage
...
### Overview
Core operations management with batch-first tracking. Template-based production orders with mobile execution, QR scanning, and real-time progress tracking.
...
- Integrates with: Module 13 (AI), Module 16 (Mobile/Media)
...
### Overview
Computer vision and NLP services trained on Colombian agricultural data. Provides pest detection, plant health assessment, Spanish form processing, and predictive analytics.
...
# FASE 3: ADVANCED FEATURES MODULES

## MODULE 14: COMPLIANCE & REPORTING

### Overview
Automated Colombian regulatory compliance monitoring with real-time tracking and report generation for INVIMA, ICA, and municipal requirements.
...
- Zero compliance-related shutdowns
...
### Overview
Real-time operational dashboards and predictive analytics for Colombian agricultural operations with Spanish/English reporting.
...
### Overview
Progressive Web App optimized for Colombian rural connectivity with offline capability, photo capture, AI analysis, and QR scanning.
...
- Average photo sync time: <30 seconds per batch
...
### Overview
Comprehensive REST API and integrations for Colombian agricultural equipment, laboratories, and market data with Spanish documentation.
...
- Colombian laboratory formats (INVIMA, ICA)s
...
- Equipment integration success rate: >80%
...
*For detailed technical specifications, see [Engineering PRD v4.1]*
*For complete database schema, see [Database Schema with Prisma]*